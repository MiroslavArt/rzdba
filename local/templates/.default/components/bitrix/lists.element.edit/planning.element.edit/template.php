...
// вставляем свои элементы
if($arResult['EXTRA_FIELDS']) {
    $notification = '<p>';
    foreach ($arResult['EXTRA_FIELDS'] as $key => $item) {
        $notification .= '<b>'.$item['NAME'].'</b>'.' '.$item['VALUE'].'.';
    }
    $notification .= '</p>';
}
...
//\Bitrix\Main\Diag\Debug::writeToFile($arTabs, "tabs", "__miros.log");
$APPLICATION->IncludeComponent(
