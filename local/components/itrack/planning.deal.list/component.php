...
if(!isset($arSort['nearest_activity']))
{
    ...
    // добавляем свои фильтры - вариант не подошел, ибо фильтр не работает с query
    /*$queryfilter = $arFilter;
	unset($queryfilter['PRESET_ID']);
    unset($queryfilter['FILTER_ID']);
    unset($queryfilter['FILTER_APPLIED']);
    unset($queryfilter[]);
    $queryfilter['!'.ROUTE_UF] = false;
    \Bitrix\Main\Diag\Debug::writeToFile($arFilter, "filter", "__miros.log");
    \Bitrix\Main\Diag\Debug::writeToFile($arSelect, "select", "__miros.log");
    $db = \Bitrix\Crm\DealTable::query()
        ->setFilter($queryfilter)
        ->setGroup([ROUTE_UF])
        ->setSelect([ROUTE_UF])
        ->exec();
    $routes = [];
    $start = $navListOptions['QUERY_OPTIONS']['OFFSET'];
    $routescount = $navListOptions['QUERY_OPTIONS']['LIMIT'];
    $finish = $start + $routescount - 1;
    $count = 1;
    while ($arDeal = $db->fetch()) {
        if($count >= $start && $count <= $finish) {
            $routes[] = $arDeal[ROUTE_UF];
        } elseif ($count>$finish) {
            break;
        }
    }
    ...
    if(!array_key_exists($arSelect[PROB_UF])) {
        array_push($arSelect, PROB_UF);
    }
	...
	while($arDeal = $dbResult->GetNext())
	{
        $group = $arDeal[ROUTE_UF];
	    if($group != $lastgroup) {
	        if($countr<$start) {
                $lastgroup = $group;
                $countr++;
	            continue;
            } elseif($countr>= $start && $countr< $finish) {
	            $creategrouping = true;
                $lastgroup = $group;
                $countr++;
            } else {
        ...
        if($creategrouping) {
            $groupdeal = [
                ...
                'draggable' => false,
                'expand' => false,
                'custom' => "<div class='tasks-grid-wrapper'>Маршрут</div>",
                'attrs' => [
                    'data-type' => 'group',
                    'data-group-id' => $lastgroup,
        ...
        $arResult['DEAL'][$arDeal['ID']]['expand'] = false;
